...
template <typename T>
class PriorBoxINT8MKLDNNOpKernel : public paddle::framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    // MKLDNN int8 uses NHWC format instead of NCHW, so the dims had to be
    // changed compared to the reference implementation
    auto feature_width = input->dims()[3];
    auto feature_height = input->dims()[2];
    if (input->layout() == framework::DataLayout::kNHWC) {
      feature_width = input->dims()[2];
...
REGISTER_OP_KERNEL(prior_box, MKLDNN, ::paddle::platform::CPUPlace,
                   ops::PriorBoxINT8MKLDNNOpKernel<float>,
                   ops::PriorBoxINT8MKLDNNOpKernel<uint8_t>,
                   ops::PriorBoxINT8MKLDNNOpKernel<int8_t>);
