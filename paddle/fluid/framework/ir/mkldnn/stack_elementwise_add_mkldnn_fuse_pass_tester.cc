...
Scope* CreateParamScope(int elementwise_add_count) {
  auto param_scope = new Scope();
  AddVarToScope(param_scope, "stack_in", {5, 1, 10, 10});
  AddVarToScope(param_scope, "stack_out", {5, 12, 10, 10});
  for (int i = 0; i < elementwise_add_count; i++) {
    AddVarToScope(param_scope, "eltwise_add_x"+std::to_string(i), {5, 12, 10, 10});
    AddVarToScope(param_scope, "eltwise_add_out"+std::to_string(i), {5, 12, 10, 10});
  }
  return param_scope;
}

void TestMain(int elementwise_add_count = 1) {
...
TEST(StackElementwiseAddMkldnnFusePass, 1_stack_12_elementwise_add) {
  TestMain();
}
...
USE_PASS(stack_elementwise_add_mkldnn_fuse_pass);
