...
std::map<std::string, std::vector<ir::Node *>> Graph::InitFromProgram(
  ...
  VLOG(3) << "blocks in program:" << program_.Size();
  ...
  for (int i = 0; i < program.Size(); i++) {
    for (auto *op : program.Block(i).AllOps()) {
  ...
  auto *all_ops = new std::vector<OpDesc *>(program.Block(0).AllOps());
  for (int i = 1; i < program.Size(); i++) {
    all_ops->insert(all_ops->end(), program.Block(i).AllOps().begin(),
                    program.Block(i).AllOps().end());
  }
  Set<const std::vector<OpDesc *>>(details::kStaleProgramOpDescs, all_ops);
  return var_nodes;
}
